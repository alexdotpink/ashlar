...
| `ashlar-config` | `ashlar-config` | Immutable typed settings, validation, migrations, lossless formats, reload, updates, backups, and inspection | Plug-ins with human-edited settings |
| `ashlar-config-test` | `ashlar-config-test` | Production configuration handles in a server-free real-filesystem harness | Tests only |
...
| `ashlar-config-ksp` | `ashlar-config-ksp` | Configuration metadata, KDoc, serializers, validation and migration calls, and pre-lifecycle linkage | KSP processor, selected by `config()` |
...
`sample-plugin` is a playable feature catalogue with a benchmark example. `integration-test-fixture` is an automated Paper/Folia workload. Neither is a published runtime artifact.
