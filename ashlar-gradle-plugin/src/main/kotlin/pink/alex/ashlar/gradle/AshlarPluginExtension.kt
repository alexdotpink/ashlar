...
/** Managed descriptor and dependency settings for an Ashlar plug-in. */
public abstract class AshlarPluginExtension @Inject constructor(private val project: Project) {
    ...
    internal val ashlarVersion: Property<String> = project.objects.property(String::class.java)
        .convention(AshlarVersions.ashlar)

    internal val paperApiVersion: Property<String> = project.objects.property(String::class.java)
        .convention(AshlarVersions.paper)
    ...
    public fun commands(strictDocumentation: Boolean = false) {
        ...
        project.extensions.configure(KspExtension::class.java) { ksp ->
            ksp.arg("ashlar.commands.strictDocumentation", strictDocumentation.toString())
        }
    }
    ...
    /** Overrides the aligned Ashlar release with one exact version. */
    public fun ashlarVersion(version: String) {
        requireOverrideAllowed()
        val exactVersion = version.requireExactVersion("ashlar")
        project.logger.warn(
            "Using Ashlar version {} outside the tested alignment {}.",
            exactVersion,
            AshlarVersions.ashlar,
        )
        ashlarVersion.set(exactVersion)
    }
