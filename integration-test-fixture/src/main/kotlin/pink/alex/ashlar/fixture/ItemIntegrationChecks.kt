...
import pink.alex.ashlar.items.CustomItemCodec
import pink.alex.ashlar.items.CustomItemDefinition
import pink.alex.ashlar.items.CustomItemRead
...
import pink.alex.ashlar.items.HmacKey
import pink.alex.ashlar.items.HmacKeyring
import pink.alex.ashlar.items.PersistentValueCodecs
import pink.alex.ashlar.items.PersistentValueRead
import pink.alex.ashlar.items.customItem
...
internal fun runItemIntegrationChecks() {
    ...
    val decoded = ItemSnapshot.decode(snapshot.encode()) as? pink.alex.ashlar.items.ItemSnapshotDecode.Found
    ...
    val legacyEnvelope = checkNotNull(
        tokenStack.persistentDataContainer.get(CustomItemDefinition.ENVELOPE_KEY, PersistentDataType.BYTE_ARRAY),
    )
    tokenStack.editPersistentDataContainer {
        it.remove(CustomItemDefinition.ENVELOPE_KEY)
        it.set(NamespacedKey("framework", "custom_item"), PersistentDataType.BYTE_ARRAY, legacyEnvelope)
    }
    check((token.read(tokenStack) as? CustomItemRead.Found)?.data == "market") {
        "Custom items written before the Ashlar rename were not recognized"
    }
